...
/**
  ...
  @param {Object} Options to apply when adding a user i.e 'expiresIn'
  @param {Function} Callback called when finished.
  @return {Promise} Promise resolved when finished
*/
Acl.prototype.addUserRoles = function(userId, roles, options, cb){
  if (!options) {
    options = {};
  }
  contract(arguments)
    .params('string|number','string|array','object','function')
    .params('string|number','string|array','object')
    .end();

  var transaction = this.backend.begin();
  this.backend.add(transaction, this.options.buckets.meta, 'users', userId, options);
  this.backend.add(transaction, this.options.buckets.users, userId, roles, options);

  if (Array.isArray(roles)) {
    var _this = this;

      roles.forEach(function(role) {
        _this.backend.add(transaction, _this.options.buckets.roles, role, userId, options);
      });
  }
  else {
      this.backend.add(transaction, this.options.buckets.roles, roles, userId, options);
  }
